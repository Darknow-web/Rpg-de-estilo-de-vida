...
export function activeOfType(missions: Mission[], type: MissionType): Mission[] {
  ...
  // Las tareas planificadas desde la lista de pendientes (origin 'agenda') tampoco: las limita el propio calendario.
  return missions.filter((m) => m.active && m.type === type && m.mastery.state !== 'automated' && (m.type !== 'hidden' || m.revealed) && m.origin !== 'calendar' && m.origin !== 'agenda');
}
