...
- **La IA y las fotos.** La IA **no** analiza las fotos de evidencia: solo prueban ante ti que hiciste la misión y se guardan como bytes en tu subárbol de Firestore, al que nadie más accede. Lo único que la IA mira son **fotos de configuración**: las del gimnasio (`/api/ai/gym-scan`: hasta 4 fotos por escaneo, 10 escaneos al día por jugador) para marcar el equipamiento del catálogo, y la de tu **lista de pendientes** (`/api/ai/tasks-from-photo`, 10 al día) para convertirla en tareas. Viajan comprimidas en la petición, se envían a Gemini y se descartan. **No se almacenan en ningún sitio** (ni Firestore, ni servidor, ni registros). Si la IA no está, la app ofrece marcar el equipamiento o escribir las tareas a mano.
- **La agenda y Google Calendar.** Los eventos se leen desde el navegador y nunca se guardan en Firestore. Al planificar pendientes (`/api/ai/plan-week`, 20 al día), a Gemini solo van título, día y horas de lo que ya ocupa la semana, más los huecos libres. La app escribe en tu calendario **solo las tareas que confirmas** (eventos con prefijo `[LQ]`) y nunca mueve ni borra nada sin que lo aceptes uno a uno. Cada tarea planificada es una misión con foto obligatoria: da XP y monedas por `awardRewards()` y, si vence, cuesta un corazón como cualquier otra.
